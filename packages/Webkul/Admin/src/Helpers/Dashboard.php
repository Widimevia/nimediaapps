...
class Dashboard
{
    ...
    public function getLeadValue($startDateFilter, $endDateFilter, $totalWeeks)
    {
        $topLeads = $this->leadRepository
            ->select(DB::raw("SUM(lead_value) as total"))
            ->where('lead_pipeline_stage_id', '=', '5')
            ->limit(3)
            ->get();
    ...
    public function getLostValue($startDateFilter, $endDateFilter, $totalWeeks)
    {
        
        $topLeads = $this->leadRepository
        ->select(DB::raw("(COUNT(DISTINCT leads.id)) as count"),DB::raw("SUM(lead_value) as lost"))
        ->where('lead_pipeline_stage_id', '=', '6')
        ->limit(3)
        ->get();
